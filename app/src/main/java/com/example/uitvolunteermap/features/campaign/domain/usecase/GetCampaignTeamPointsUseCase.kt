...
class GetCampaignTeamPointsUseCase @Inject constructor(
    private val repository: CampaignAreaRepository
) {
    suspend operator fun invoke(campaignId: Int): AppResult<List<TeamVisitPoint>> =
        repository.getTeamPoints(campaignId)
}
